...
public interface RapportRepository extends JpaRepository<Rapport, Long> {
        ...
        // Recherche multicritères - utilise LEFT JOIN et EntityGraph pour éviter N+1 et
        // LazyInitializationException
        @org.springframework.data.jpa.repository.EntityGraph(attributePaths = { "projet" })
        @Query(value = "SELECT r FROM Rapport r LEFT JOIN r.projet p WHERE " +
                        "(:nom IS NULL OR :nom = '' OR LOWER(r.nom) LIKE LOWER(CONCAT('%', :nom, '%'))) AND " +
                        "(:projetId IS NULL OR p.id = :projetId) AND " +
                        "(:fichierType IS NULL OR :fichierType = '' OR r.fichierType = :fichierType) AND " +
                        "(:uploadePar IS NULL OR :uploadePar = '' OR LOWER(r.uploadePar) LIKE LOWER(CONCAT('%', :uploadePar, '%'))) AND "
                        +
                        ...
                        "(CAST(:uploadDateFrom AS java.time.LocalDateTime) IS NULL OR r.createdAt >= :uploadDateFrom) AND "
                        +
                        "(CAST(:uploadDateTo AS java.time.LocalDateTime) IS NULL OR r.createdAt <= :uploadDateTo)")
